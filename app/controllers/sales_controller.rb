class SalesController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to sales_url, notice: 'Venda excluída.'}
  ...
  def update_line_item_options
    ...
    if params[:search][:item_category].blank? 
      @available_items = Item.find(:all, :conditions => ['name LIKE ? AND published = true OR description LIKE ? AND published = true OR sku LIKE ? AND published = true', "%#{params[:search][:item_name]}%", "%#{params[:search][:item_name]}%", "%#{params[:search][:item_name]}%"], :limit => 5)
    elsif params[:search][:item_name].blank?
      @available_items = Item.where(:item_category_id => params[:search][:item_category]).limit(5)
    else
      @available_items = Item.find(:all, :conditions => ['name LIKE ? AND published = true AND item_category_id = ? OR description LIKE ? AND published = true AND item_category_id = ? OR sku LIKE ? AND published = true AND item_category_id = ?', "%#{params[:search][:item_name]}%", "#{params[:search][:item_category]}", "%#{params[:search][:item_name]}%", "#{params[:search][:item_category]}", "%#{params[:search][:item_name]}%", "#{params[:search][:item_category]}"], :limit => 5)
    end
